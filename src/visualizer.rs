...
pub fn draw_xbox_controller(ui: &mut egui::Ui, gamepad: &GamepadState) {
    ...
    painter.text(top_rect.min + Vec2::new(5.0, 0.0), egui::Align2::LEFT_TOP, "TOP VIEW", egui::FontId::proportional(10.0), text_color);

    let top_body_rect = top_rect.shrink2(Vec2::new(20.0, 10.0)).translate(Vec2::new(0.0, 5.0));
    ...
    let draw_stick = |c: Pos2, x: f32, y: f32, lbl: &str| {
        ...
        let col = if act { Color32::from_rgb(0, 255, 255) } else { Color32::from_rgb(60, 60, 65) };
        painter.circle_filled(head, 14.0, col);
        painter.circle_stroke(head, 14.0, Stroke::new(1.0, outline_color));
        painter.text(c + Vec2::new(0.0, 35.0), egui::Align2::CENTER_TOP, lbl, egui::FontId::proportional(12.0), text_color);
    };
